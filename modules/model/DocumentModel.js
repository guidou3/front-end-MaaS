...
class DocumentModel {
  constructor(params, populate, bodyRows) {
    ...
    AttributeReader.readRequiredAttributes(params, this, ["collection"], function(param) {
      self.err = ("Required parameter '" + param + "' in document '" + self.toString() + "'");
    });
  ...
  render(store) {
    try{
      ...
      if(this.flag){                                                              //EXECUTES ONCE
        ...
        executeQuery(store.getState().currentDSLI, query, token, (err,res) =>{                                 //LAUNCH OF A QUERY
